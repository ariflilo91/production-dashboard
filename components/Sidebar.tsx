...
function SectionLabel({ text }: { text: string }) {
  return (
    <div style={{ fontSize: 9, color: '#5F5E5A', textTransform: 'uppercase', letterSpacing: '.09em', fontWeight: 700, padding: '0 8px', marginBottom: 5, marginTop: 4 }}>
      {text}
    </div>
  )
}

// Fix 2: Logo as an image URL — easy to change
...
export default function Sidebar({ projects = [], activeProjectId }: SidebarProps) {
  ...
  return (
    <div style={{ width: 220, minWidth: 220, background: '#111110', borderRight: '1px solid #222220', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0, overflowY: 'auto' }}>
      {/* Logo — Fix 2: show image logo, fallback to icon */}
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #222220' }}>
        <div style={{ width: 44, height: 44, borderRadius: 10, background: '#0d2a45', border: '1px solid #1a4060', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10, overflow: 'hidden' }}>
          <img
            src={LOGO_URL}
            alt="Durioo logo"
            style={{ width: '100%', height: '100%', objectFit: 'cover' }}
            onError={e => {
              // fallback: hide broken image, show SVG
          ...
          />
          <svg style={{ display: 'none' }} width="20" height="20" viewBox="0 0 14 14" fill="none">
      ...
      <div style={{ padding: 8, borderTop: '1px solid #222220' }}>
        <Link href="/new-project" style={{ textDecoration: 'none' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '7px 8px', borderRadius: 7, border: '1.5px dashed #2a2a27', cursor: 'pointer', color: '#888780', fontSize: 11, fontWeight: 600 }}>
