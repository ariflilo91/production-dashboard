...
function HolidaysModal({ holidays, projectId, onAdd, onRemove, onUpdate, onClose }: {
  ...
  const rowInp: React.CSSProperties = { height: 26, padding: '0 7px', borderRadius: 5, border: '1px solid #378ADD', background: 'var(--blue-bg)', color: 'var(--text-primary)', fontSize: 11, fontFamily: 'inherit' }

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 560, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 48px rgba(0,0,0,.6)' }}>
        ...
        <div style={{ fontSize: 10, color: 'var(--text-faint)', marginBottom: 10 }}>Click any row to edit inline</div>
        <div style={{ flex: 1, overflowY: 'auto', marginBottom: 14 }}>
          {holidays.length === 0
            ...
            : [...holidays].sort((a, b) => a.date.localeCompare(b.date)).map(h => {
              ...
              return (
                <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 8, background: isEditing ? 'var(--blue-bg)' : 'var(--bg-surface)', marginBottom: 5, borderLeft: `3px solid ${h.type === 'ph' ? '#854F0B' : '#534AB7'}`, cursor: isEditing ? 'default' : 'pointer', transition: 'background 0.12s' }}
                  ...
                  {isEditing ? (
                    <>
                      ...
                      <button onClick={saveEdit} disabled={editSaving} style={{ height: 26, padding: '0 10px', borderRadius: 5, background: 'var(--blue-bg)', color: 'var(--blue)', border: '1px solid #1a4060', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                        {editSaving ? '...' : 'Save'}
                      </button>
                      <button onClick={() => setEditId(null)} style={{ height: 26, padding: '0 8px', borderRadius: 5, background: 'transparent', color: 'var(--text-dim)', border: '1px solid var(--border)', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                    </>
                  ) : (
                    <>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: h.type === 'ph' ? 'var(--amber-bg)' : '#1a0a2a', color: h.type === 'ph' ? 'var(--amber)' : '#AFA9EC', whiteSpace: 'nowrap' }}>
                  ...
                  {!isEditing && (
                    <button onClick={e => { e.stopPropagation(); deleteHoliday(h.id); onRemove(h.id) }} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: 13, padding: '0 2px' }}>✕</button>
                  )}
                </div>
              )
            })
...
function EpisodeModal({ episodes, projectId, onAdd, onDelete, onClose }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 440, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 48px rgba(0,0,0,.6)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ borderTop: '1px solid var(--border-sub)', paddingTop: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            <button onClick={add} disabled={saving} style={{ height: 34, padding: '0 16px', borderRadius: 7, background: 'var(--green-bg)', color: 'var(--green)', border: '1px solid #183018', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              {saving ? '...' : '+ Add'}
            </button>
          </div>
        </div>
...
function TaskModal({ modal, departments, episodes, teamMembers, isAdmin, onSave, onDelete, onClose }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 420, boxShadow: '0 8px 48px rgba(0,0,0,.5)' }}>
        ...
        {isAdmin && (
          <>
            ...
            <select value={assignedTo} onChange={e => setAssignedTo(e.target.value)} style={{ ...modalInp, marginBottom: 12 }}>
              ...
              {teamMembers.map(m => <option key={m.id} value={m.id}>{m.display_name || m.email}</option>)}
            </select>
          </>
        )}
        ...
        {error && <div style={{ background: 'var(--red-bg)', border: '1px solid #3a1010', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: 'var(--red)', marginBottom: 14 }}>{error}</div>}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={onClose} style={{ height: 38, padding: '0 16px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <TopNav
          ...
          actions={
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <button onClick={() => setModal({ type: 'add' })} style={{ height: 28, padding: '0 14px', borderRadius: 7, background: 'var(--green-bg)', color: 'var(--green)', border: '1px solid #183018', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                + Add task
              </button>
            </div>
          }
        ...
        <div style={{ padding: '14px 20px', overflowY: 'auto', flex: 1 }}>
          ...
          {/* Gantt */}
          <div style={{ border: '1px solid var(--border-sub)', borderRadius: 10, overflow: 'hidden', background: 'var(--bg-surface)' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ borderCollapse: 'collapse', fontSize: 11, minWidth: '100%' }}>
                <thead>
                  <tr>
                    <th rowSpan={2} style={{ position: 'sticky', left: 0, zIndex: 8, background: 'var(--bg-surface)', minWidth: 150, maxWidth: 150, padding: '7px 10px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '.06em', borderRight: '1px solid var(--border-sub)', borderBottom: '1px solid var(--border-sub)' }}>
                      Department
                    </th>
                    {months.map((m, i) => (
                      ...
                      return (
                        <th
                          ...
                          style={{
                            ...
                            borderRight: '1px solid var(--border-dim)', borderBottom: '1px solid var(--border-sub)',
                ...
                <tbody>
                  {GROUPS.map(grp => {
                    ...
                    return [
                      <tr key={grp.label}>
                        <td style={{ padding: '5px 10px', background: 'var(--bg-base)', fontSize: 9, fontWeight: 700, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '.09em', borderBottom: '1px solid var(--border-dim)', borderRight: '1px solid var(--border-sub)', position: 'sticky', left: 0, zIndex: 4, whiteSpace: 'nowrap', minWidth: 150, maxWidth: 150 }}>
                          {grp.label}
                        </td>
                        <td colSpan={total} style={{ background: 'var(--gantt-group-bg)', borderBottom: '1px solid var(--border-dim)' }} />
                      </tr>,
                      ...
                      ...grpDepts.map(dept => {
                        ...
                        if (lanes.length === 0) {
                          return (
                            <tr key={dept.id} style={{ borderBottom: '1px solid var(--border-dim)' }}>
                              <td style={{ position: 'sticky', left: 0, zIndex: 2, background: 'var(--bg-surface)', padding: '0 10px', minWidth: 150, maxWidth: 150, height: 36, verticalAlign: 'middle', borderRight: '1px solid var(--border-sub)' }}>
                              ...
                              {Array.from({ length: total }).map((_, i) => {
                                ...
                                return (
                                  <td key={i} style={{ minWidth: COL_W, width: COL_W, height: 36, padding: 0, position: 'relative', borderRight: '1px solid var(--border-dim)' }}>
                        ...
                        return lanes.map((laneEps, laneIdx) => (
                          <tr key={`${dept.id}-lane-${laneIdx}`} style={{ borderBottom: '1px solid var(--border-dim)' }}>
                            {laneIdx === 0 && (
                              <td rowSpan={lanes.length} style={{ position: 'sticky', left: 0, zIndex: 2, background: 'var(--bg-card)', padding: '0 10px', minWidth: 150, maxWidth: 150, verticalAlign: 'middle', borderRight: '1px solid var(--border-sub)' }}>
      ...
      {/* Tooltip */}
      {tooltip && (
        <div style={{ position: 'fixed', left: Math.min(tooltip.x + 14, window.innerWidth - 220), top: Math.min(tooltip.y - 10, window.innerHeight - 170), zIndex: 999, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', fontSize: 11, minWidth: 200, pointerEvents: 'none', boxShadow: '0 4px 24px rgba(0,0,0,.5)' }}>
...
}// ─── Holidays Modal — Fix 1: inline editable ─────────
function HolidaysModal({ holidays: initHolidays, projectId, onAdd, onRemove, onClose }: {
  holidays: Holiday[]; projectId: string
  ...
  async function saveEdit() {
    ...
    setSaving(true)
    try {
      const saved = await upsertHoliday({ id: editId, project_id: projectId, date: editDate, name: editName, type: editType })
      const updated = holidays.map(h => h.id === editId ? saved : h)
      setHolidays(updated)
      setEditId(null)
    } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error') }
    setSaving(false)
  ...
  async function add() {
    ...
    try {
      const saved = await upsertHoliday({ project_id: projectId, date, name: name.trim(), type })
      const updated = [...holidays, saved].sort((a,b) => a.date.localeCompare(b.date))
      setHolidays(updated); onAdd(saved); setDate(''); setName(''); setError('')
    } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error') }
    setSaving(false)
  }
  ...
  const rowInp: React.CSSProperties = { height: 28, padding: '0 8px', borderRadius: 5, border: '1px solid #378ADD', background: 'var(--blue-bg)', color: 'var(--text-primary)', fontSize: 11, fontFamily: 'inherit' }

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 540, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 48px rgba(0,0,0,.6)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>Holidays &amp; special leave</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', fontSize: 18 }}>✕</button>
        </div>
        <div style={{ fontSize: 11, color: 'var(--text-faint)', marginBottom: 14 }}>Click any row to edit it inline.</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', marginBottom: 14 }}>
          {holidays.length === 0
            ...
            : [...holidays].sort((a,b) => a.date.localeCompare(b.date)).map(h => {
              const isEditing = editId === h.id
              return (
                <div key={h.id} onClick={() => !isEditing && startEdit(h)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 8, background: isEditing ? 'var(--blue-bg)' : 'var(--bg-surface)', marginBottom: 5, borderLeft: `3px solid ${h.type === 'ph' ? '#854F0B' : '#534AB7'}`, cursor: isEditing ? 'default' : 'pointer', border: isEditing ? '1px solid #1a4060' : 'none', borderLeft: `3px solid ${h.type === 'ph' ? '#854F0B' : '#534AB7'}` }}>
                  {isEditing ? (
                    <>
                      <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)} style={{ ...rowInp, width: 130 }} />
                      <input type="text" value={editName} onChange={e => setEditName(e.target.value)} style={{ ...rowInp, flex: 1 }} autoFocus />
                      <select value={editType} onChange={e => setEditType(e.target.value as 'ph'|'sl')} style={{ ...rowInp, width: 110 }}>
                        <option value="ph">Public holiday</option>
                        <option value="sl">Special leave</option>
                      </select>
                      <button onClick={saveEdit} disabled={saving} style={{ height: 28, padding: '0 12px', borderRadius: 5, background: 'var(--green-bg)', color: 'var(--green)', border: '1px solid #183018', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>Save</button>
                      <button onClick={() => setEditId(null)} style={{ height: 28, padding: '0 8px', borderRadius: 5, background: 'transparent', color: 'var(--text-dim)', border: '1px solid var(--border)', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>✕</button>
                    </>
                  ) : (
                    <>
                      <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', minWidth: 88 }}>
                        {new Date(h.date + 'T12:00:00').toLocaleDateString('en-MY', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </span>
                      <span style={{ flex: 1, fontSize: 11, color: 'var(--text-dim)' }}>{h.name}</span>
                      <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: h.type === 'ph' ? 'var(--amber-bg)' : '#1a0a2a', color: h.type === 'ph' ? 'var(--amber)' : '#AFA9EC', whiteSpace: 'nowrap' }}>
                      ...
                      <span style={{ fontSize: 9, color: 'var(--text-faint)', marginLeft: 4 }}>✎</span>
                      <button onClick={e => { e.stopPropagation(); remove(h.id) }} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: 14, padding: '0 2px' }}>✕</button>
                    </>
                  )}
                </div>
              )
...
export default function ProjectPage({ params }: { params: { project: string } }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      {modal && <TaskModal modal={modal} departments={departments} episodes={episodes} onSave={saveTask} onDelete={removeTask} onClose={() => setModal(null)} />}
      {showHolidays && <HolidaysModal holidays={holidays} projectId={projectId} onAdd={h => setHolidays(prev => [...prev, h].sort((a,b) => a.date.localeCompare(b.date)))} onRemove={id => setHolidays(prev => prev.filter(h => h.id !== id))} onUpdate={h => setHolidays(prev => prev.map(x => x.id === h.id ? h : x))} onClose={() => setShowHolidays(false)} />}
>>>>>>> parent of aadfcff (v8: fix - clean CSS vars, no duplicate exports)
      {showEpisodes && <EpisodeModal episodes={episodes} projectId={projectId} onAdd={ep => setEpisodes(prev => [...prev, ep].sort((a, b) => a.name.localeCompare(b.name)))} onDelete={id => setEpisodes(prev => prev.filter(e => e.id !== id))} onClose={() => setShowEpisodes(false)} />}
    </div>
  )
}
